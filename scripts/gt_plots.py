...
from exp1_plots import TRAJECTORY_TYPE, TrajectoryType  # reuse the same TrajectoryType enum
...
def _plot_trajectories(asv_pos: np.ndarray, rov_pos: np.ndarray, t: np.ndarray,
    ...
    ax.legend(loc="upper left", fontsize="small", framealpha=0.4)
    ...
    ax2.legend(loc="best", fontsize="small", framealpha=0.4)
...
def _plot_geometry(t: np.ndarray, bearing: np.ndarray, heading: np.ndarray, slant_range: np.ndarray,
    ...
    fig, axs = plt.subplots(
        3, 1,
        figsize=(3.4, 3.8),   # final subfigure size, not large then scaled down
        sharex=True,
        constrained_layout=True
    )
    ...
    # Optional: remove this if the LaTeX caption already explains it
    # fig.suptitle(f"Measurement geometry of {title_traj}")
...
def main():
    ...
    parser.add_argument("--trajectory", type=str, default="linear_turns",
    ...
    fig_pre ="fig8" if traj_type == TrajectoryType.FIGURE_8 else "linear"
    out_dir = Path(f"plots_new/{traj_type.value}")
    ...
    traj_path = out_dir / f"{fig_pre}_sim_trajectories.svg"
    geom_path = out_dir / f"{fig_pre}_sim_geometry.svg"
