...
from tracking_only.states import RovNominalState, RovErrorState, RovEskfState, ASVState
...
class _ESKFUpdateShared:
    ...
    def _update_err(
        self,
        rov_est_pred: RovEskfState,
        z_pred_gauss: MultiVarGauss,
        z_measurement: Any,
        H: np.ndarray,
        R: np.ndarray,
        is_usbl: bool = False,
    ) -> MultiVarGauss[RovErrorState]:
        ...
        return MultiVarGauss[RovErrorState](
            RovErrorState.from_array(rov_err_upd_mean), rov_err_cov_upd
        )

    @staticmethod
    def _inject(
        rov_est_nom: RovNominalState,
        rov_est_err: MultiVarGauss[RovErrorState],
    ) -> RovEskfState:
        ...
        x_err_inj = MultiVarGauss[RovErrorState](np.zeros(15), P_inj)
        return RovEskfState(x_nom_inj, x_err_inj)
...
class ESKF_cv(_ESKFUpdateShared):
    ...
    def predict_with_cv(self,
        ...
        x_est_pred = RovEskfState(x_est_pred_nom, x_est_pred_err)
    ...
    def update_from_usbl(self, 
                        rov_est_pred: RovEskfState, 
                        asv_state: ASVState, 
                        z_usbl: UsblMeasurement) -> Tuple[RovEskfState, MultiVarGauss]:
    ...
    def update_from_range(self, 
                            rov_est_pred: RovEskfState, 
                            asv_state: ASVState, 
                            z_range: RangeMeasurement) -> Tuple[RovEskfState, MultiVarGauss]:
    ...
    def update_from_depth(self, 
                          rov_est_pred: RovEskfState, 
                          z_depth: DepthMeasurement) -> Tuple[RovEskfState, MultiVarGauss]:
